 ...
 */
public class YourReactModle extends ReactContextBaseJavaModule {
    private Activity context;
    private ReactApplicationContext reactApplicationContext;

    public YourReactModle(final ReactApplicationContext reactContext, Activity context) {
        super(reactContext);
    ...
    public void getAddressList(final Callback successCallback, final Callback failedCallback) {
        ...
        list.add(new Modle("0", "测试地址0", true));
        list.add(new Modle("1", "测试地址1", false));
        list.add(new Modle("2", "测试地址2", false));
        WritableArray writableArray = new WritableNativeArray();
        for (Modle address : list) {
            if (address != null) {
